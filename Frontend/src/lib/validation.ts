...
export function isValidGrade(grade: string): boolean {
  return ["GRADE_1", "GRADE_2", "GRADE_3", "GRADE_4"].includes(grade);
}

export function getCourseLabel(gradeValue: string | null): string {
  if (!gradeValue) return "-";
  const matched = GRADE_OPTIONS.find((opt) => opt.value === gradeValue);
  if (matched) return matched.label;
  const normalized = gradeValue.toUpperCase().replace(/\s+/g, "");
  if (normalized === "GRADE1" || normalized === "1" || normalized === "GRADE_1") {
    return "Grade 1";
  }
  if (normalized === "GRADE2" || normalized === "2" || normalized === "GRADE_2") {
    return "Grade 2";
  }
  if (normalized === "GRADE3" || normalized === "3" || normalized === "GRADE_3") {
    return "Grade 3";
  }
  if (normalized === "GRADE4" || normalized === "4" || normalized === "GRADE_4") {
    return "Grade 4";
  }
  return gradeValue;
}
