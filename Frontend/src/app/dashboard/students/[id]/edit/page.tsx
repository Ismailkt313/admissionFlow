...
import { ArrowLeft, User, Calendar, School, Layers, AlertTriangle } from "lucide-react";
...
export default function EditStudentPage() {
  ...
  useEffect(() => {
    ...
    api.getStudentDetails(token, id)
      ...
      .catch((err) => {
        setServerError(err.message || "Failed to fetch student details.");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.updateStudent(token, id, {
      ...
      toast({
        ...
        description: `${form.studentName}'s profile has been updated successfully.`,
        variant: "success",
      });
      ...
      if (Array.isArray(error.message)) {
        ...
        setServerError("Failed to update student record. Please try again.");
      }
    } finally {
  ...
  return (
    <div className="max-w-xl mx-auto w-full">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h2 className="text-[15px] font-semibold text-slate-900">Edit Student</h2>
          <p className="text-[11px] text-slate-400 mt-0.5">
            {isReadOnly ? "This profile is locked." : `Modify details for ${form.studentName || "student"}.`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/80 rounded-lg overflow-hidden">
        <div className="px-5 py-3.5 border-b border-slate-100">
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            {isReadOnly
              ? "This student profile cannot be edited."
        ...
        <div className="p-5">
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            {isReadOnly && (
              <div className="flex items-center gap-2.5 px-3 py-2.5 bg-amber-50 border border-amber-200/60 rounded-md">
                <AlertTriangle className="h-4 w-4 text-amber-500 shrink-0" />
                <p className="text-[11px] font-medium text-amber-800">
                  Student details cannot be modified after registration fee payment.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-2.5 pt-4 border-t border-slate-100">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(isReadOnly ? `/dashboard/students/${id}` : "/dashboard/students")}
