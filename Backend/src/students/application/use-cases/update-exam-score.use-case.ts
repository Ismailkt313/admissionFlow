import { Injectable, NotFoundException, BadRequestException, ConflictException } from '@nestjs/common';
import { IStudentRepository } from '../../domain/repositories/student.repository.interface';
import { Student, ApplicationStatus, PaymentStatus } from '../../domain/entities/student.entity';
import { UpdateExamScoreDto } from '../../presentation/dto/update-exam-score.dto';

@Injectable()
export class UpdateExamScoreUseCase {
  constructor(private readonly studentRepository: IStudentRepository) {}

  async execute(id: string, dto: UpdateExamScoreDto): Promise<Student> {
    const student = await this.studentRepository.findById(id);
    if (!student) {
      throw new NotFoundException('Student not found.');
    }

    if (student.paymentStatus !== PaymentStatus.PAID) {
      throw new BadRequestException('Registration fee has not been completed.');
    }

    if (!student.examSlotId) {
      throw new BadRequestException('Entrance exam has not been booked.');
    }

    if (student.status !== ApplicationStatus.SLOT_BOOKED) {
      if (student.status === ApplicationStatus.EXAM_COMPLETED || student.status === ApplicationStatus.ADMISSION_COMPLETED) {
        throw new ConflictException('Exam score already exists.');
      }
      throw new BadRequestException('Entrance exam has not been booked.');
    }

    if (student.examScore !== null) {
      throw new ConflictException('Exam score already exists.');
    }

    const updated = await this.studentRepository.update(id, {
      examScore: dto.examScore,
      status: ApplicationStatus.EXAM_COMPLETED,
    });

    if (!updated) {
      throw new NotFoundException('Student not found.');
    }

    return updated;
  }
}
